...
std::vector<std::string> get_symbols_of_nonempty_balances(const binapi::rest::account_info_t &accinfo) {
    ...
    for (const auto &it : accinfo.balances) {
        if (it.second.free > 0 || it.second.locked > 0) { res.push_back(it.first); }
    }
...
std::vector<std::string> get_pairs_for_pairs(const rest::account_info_t     &accinfo,
    ...
    std::sort(std::begin(mpairs), std::end(mpairs), [](std::string l, std::string r) {
...
void remove_all_buy_trades(trade_info_container_t &trades) {
    ...
    for (auto &it : trades) {
        if (!it.isBuyer) { tmp.push_back(std::move(it)); }
    }
...
std::vector<cycle_pair> get_full_cycle_trades(binapi::rest::api                                     &api,
    ...
    for (auto &oit : trades) {
        ...
        std::cout << (boost::format("%-10s: ") % oit.symbol) << "B:" << buy_order_info.v.orderId
                  << " -> S:" << sell_order_info.v.orderId << " C:" << sell_order_info.v.clientOrderId << std::endl;

        cycle_pair cpair{ std::move(sell_order_info.v), std::move(buy_order_info.v) };
...
struct pair_trades {
    std::size_t cycles;
    double      total_buy_base;
    double      total_buy_quoted;
    double      total_sell_base;
    double      total_sell_quoted;

    friend std::ostream &operator<<(std::ostream &os, const pair_trades &o) {
        os << "{" << "\"cycles\":" << o.cycles << "," << "\"total_buy_base\":\"" << o.total_buy_base << "\","
           << "\"total_buy_quoted\":\"" << o.total_buy_quoted << "\"," << "\"total_sell_base\":\""
           << o.total_sell_base << "\"," << "\"total_sell_quoted\":\"" << o.total_sell_quoted << "\"" << "}";
...
void make_trades_report(std::ostream                                          &os,
    ...
    const auto mtime_from = start_time > 0 ? start_time
            : !start_time_str.empty()      ? mstime_from_str(start_time_str)
                                           : 0;
    ...
    for (const auto &pair : mpairs) {
        ...
        std::cout << (boost::format("%-10s: ") % pair)
    ...
    for (const auto &it : buy_sell_pairs) {
        ...
        os << (boost::format("%-10s: %11s") % it.first % binapi::format_double(diff, 8, iofmt)) << " "
           << syminfo.quoteAsset;
        ...
        os << " (" << it.second.cycles << " trades)" << std::endl;
    }

    if (!total_profit.empty()) {
        os << std::endl << "PROFIT RESUME:" << std::endl;

        for (const auto &it : total_profit) {
            os << "  " << it.second.profit << " " << it.first << " (BUY: " << it.second.total_buy
               << ", SELL: " << it.second.total_sell << ", TRADES: " << summ_cycles(total_profit) << ")";
...
rest::orders_info_t get_open_orders(rest::api                                      &api,
                                    ...
                                    const std::uint64_t                             start_time,
                                    const std::string                              &start_time_str) {
    const auto mstart_time = start_time > 0 ? start_time
            : !start_time_str.empty()       ? mstime_from_str(start_time_str)
                                            : 0;
    ...
    if (side) {
        ...
        for (auto &pit : orders.orders) {
            for (auto &it : pit.second) {
                if (it.side == side) { tmp.orders[it.symbol].push_back(std::move(it)); }
            }
        }
    ...
    if (mstart_time) {
        ...
        for (auto &pit : orders.orders) {
            for (auto &it : pit.second) {
                if (it.time >= mstart_time) { tmp.orders[it.symbol].push_back(std::move(it)); }
            }
        }
...
void make_open_orders_resume(std::ostream                        &os,
                             binapi::rest::orders_info_t         &orders,
    ...
    for (const auto &pit : orders.orders) {
        ...
        double      buy_for_pair(0), sell_for_pair(0);
        double      buy_amount(0), sell_amount(0);
        ...
        if (buy_count) {
            os << "  B: " << buy_count << " orders, total: " << buy_amount << " " << sym.baseAsset << " = "
               << buy_for_pair << " " << sym.quoteAsset << std::endl;
        }
        if (sell_count) {
            os << "  S: " << sell_count << " orders, total: " << sell_amount << " " << sym.baseAsset << " = "
               << sell_for_pair << " " << sym.quoteAsset << std::endl;
        }
        os << std::endl;
    }

    if (buy_total) { os << "BUY  TOTAL: " << buy_total << std::endl; }
    if (sell_total) { os << "SELL TOTAL: " << sell_total << std::endl; }
}

void make_open_orders_detailed(std::ostream                        &os,
                               binapi::rest::orders_info_t         &orders,
                               const binapi::rest::exchange_info_t &exinfo) {
    (void)exinfo;

    static const auto print_buy_or_sell_orders_info =
            [](std::ostream &os, bool buy_or_sell, const std::vector<rest::order_info_t> &orders) {
                for (const auto &oit : orders) {
                    if (buy_or_sell) {
                        if (oit.side[0] == 'B') {
                            os << boost::format(" B: %10u|%10g|%8g|%10g|") % oit.orderId % oit.price % oit.origQty %
                                            (oit.price * oit.origQty)
                               << std::endl;
                        }
                    } else {
                        if (oit.side[0] == 'S') {
                            os << boost::format(" S: %10u|%10g|%8g|%10g|") % oit.orderId % oit.price % oit.origQty %
                                            (oit.price * oit.origQty)
                               << std::endl;
                        }
                    }
    ...
    for (auto &pit : orders.orders) {
        os << pit.first << ":" << std::endl;
        std::sort(pit.second.begin(),
                  pit.second.end(),
                  [](const rest::order_info_t &l, const rest::order_info_t &r) { return l.side < r.side; });
...
void show_exchanger_price_for_orders(std::ostream                                   &os,
    ...
    struct out_item {
        std::string         pair;
        double              exchanger_price;
        std::vector<double> order_price;
    };
    std::vector<out_item> out_items;

    for (auto &it : orders.orders) {
        std::sort(it.second.begin(), it.second.end(), [](const rest::order_info_t &l, const rest::order_info_t &r) {
    ...
    std::sort(out_items.begin(), out_items.end(), [](const out_item &l, const out_item &r) {
    ...
    for (const auto &it : out_items) {
        const auto price_prec =
                binapi::num_fractions_from_double(exinfo.get_by_symbol(it.pair).get_filter_price().tickSize);
        os << it.pair << ": " << binapi::format_double(it.exchanger_price, price_prec, iofmt) << ", "
           << binapi::format_double(percents_diff(prices.get_by_symbol(it.pair).price, it.order_price.front()),
                                    2,
                                    iofmt)
           << "% : ";
        for (auto oit = it.order_price.begin(); oit != it.order_price.end(); ++oit) {
            os << binapi::format_double(*oit, price_prec, iofmt);
            if (std::next(oit) != it.order_price.end()) { os << ", "; }
        }
        os << std::endl;
    }
}
...
void calc_loss_for_orders(std::ostream                                   &os,
    ...
    for (const auto &it : out_items) {
        ...
        const auto  price_prec =
                binapi::num_fractions_from_double(exinfo.get_by_symbol(it.pair).get_filter_price().tickSize);
        os << it.pair << ": " << binapi::format_double(it.loss, price_prec, iofmt) << " " << sym.quoteAsset
           << std::endl;
    }
    os << "   TOTAL LOSS: " << binapi::format_double(total_loss, 2, iofmt) << std::endl;
}
