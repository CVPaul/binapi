...
#define __CATCH_BLOCK_CB(unused0, data, elem)                                                                       \
    BOOST_PP_IF(BOOST_PP_GREATER(BOOST_PP_TUPLE_SIZE(elem), 1),                                                     \
...
#define __CATCH_BLOCK(os, seq) BOOST_PP_SEQ_FOR_EACH(__CATCH_BLOCK_CB, os, BOOST_PP_CAT(__CATCH_BLOCK_WRAP_X seq, 0))

#define __TRY_BLOCK() try
...
std::string hmac_sha256(const char *key, std::size_t klen, const char *data, std::size_t dlen) {
    ...
    auto p = ::HMAC(::EVP_sha256(), key, klen, (std::uint8_t *)data, dlen, digest, &dilen);
    assert(p);
...
bool verify_signature(const unsigned char *sig, std::size_t slen, const char *data, std::size_t dlen) {
    ...
    if (1 !=
        (EVP_DigestInit_ex(ctx, md, nullptr) && EVP_DigestVerifyInit(ctx, nullptr, md, nullptr, vkey) &&
         EVP_DigestVerifyUpdate(ctx, data, dlen) && EVP_DigestVerifyFinal(ctx, sig, slen))) {
        std::cerr << "EVP_DigestVerify* failed!" << std::endl;
        result = false;
    }

    if (pubkeyfile) BIO_free(pubkeyfile);
...
std::string rsa_sha256(const char *privkeyfile, std::size_t /*pklen*/, const char *data, std::size_t dlen) {
    ...
    if (1 !=
        (EVP_DigestSignInit(mdctx, nullptr, ::EVP_sha256(), nullptr, pkey) &&
         EVP_DigestSignUpdate(mdctx, data, dlen) && EVP_DigestSignFinal(mdctx, nullptr, &req))) {
    ...
    if (1 != EVP_DigestSignFinal(mdctx, signature, &slen)) { std::cerr << "Digest Final (2) failed" << std::endl; }
    assert(slen == req);
...
struct api::impl {
    impl(boost::asio::io_context &ioctx,
        ...
        : m_ioctx{ ioctx }, m_host{ std::move(host) }, m_port{ std::move(port) }, m_pk{ std::move(pk) },
          m_sk{ std::move(sk) }, m_timeout{ timeout }, m_client_api_string{ std::move(client_api_string) },
          m_write_in_process{}, m_async_requests{}, m_ssl_ctx{ boost::asio::ssl::context::sslv23_client },
          m_resolver{ m_ioctx } {}
    ...
    api::result<R> post(bool                                  _signed,
                        const char                           *target,
                        boost::beast::http::verb              action,
                        const std::initializer_list<kv_type> &map,
                        CB                                    cb) {
        static_assert(std::tuple_size<Args>::value == 4, "callback signature is wrong!");

        auto is_valid_value = [](const val_type &v) -> bool {
            if (const auto *p = boost::get<const char *>(&v)) { return *p != nullptr; }
            if (const auto *p = boost::get<std::size_t>(&v)) { return *p != 0u; }
        ...
        auto to_string = [](char *buf, std::size_t bufsize, const val_type &v) -> const char * {
            if (const auto *p = boost::get<const char *>(&v)) { return *p; }
        ...
        bool get_delete = action == boost::beast::http::verb::get || action == boost::beast::http::verb::delete_;
        ...
        if (!cb) {
            ...
            } catch (const std::exception &ex) { __MAKE_ERRMSG(res, ex.what()) }

            return res;
        } else {
            using invoker_type = detail::invoker<typename boost::callable_traits::return_type<CB>::type, R, CB>;
            async_req_item item{ starget, action, std::move(data), std::make_shared<invoker_type>(std::move(cb)) };
    ...
    api::result<std::string> sync_post(const char *target, boost::beast::http::verb action, std::string data) {
    ...
    void on_resolve(const boost::system::error_code             &ec,
        ...
        boost::asio::async_connect(sptr->next_layer(),
                                   results.begin(),
                                   results.end(),
                                   [this,
                                    req        = std::move(req),
                                    ssl_socket = std::move(ssl_socket)](const boost::system::error_code &ec,
                                                                        auto) mutable {
    ...
    void on_connect(const boost::system::error_code &ec, request_ptr req, ssl_socket_ptr ssl_socket) {
        ...
        sptr->async_handshake(boost::asio::ssl::stream_base::client,
    ...
    void on_handshake(const boost::system::error_code &ec, request_ptr req, ssl_socket_ptr ssl_socket) {
        ...
        boost::beast::http::async_write(*socket_ptr,
                                        *request_ptr,
                                        [this,
                                         req        = std::move(req),
                                         ssl_socket = std::move(ssl_socket)](const boost::system::error_code &ec,
                                                                             std::size_t wr) mutable {
    ...
    void on_write(const boost::system::error_code &ec, request_ptr req, ssl_socket_ptr ssl_socket, std::size_t wr) {
        ...
        boost::beast::http::async_read(*socket_ptr,
                                       m_buffer,
                                       *resp_ptr,
                                       [this,
                                        resp       = std::move(resp),
                                        ssl_socket = std::move(ssl_socket)](const boost::system::error_code &ec,
                                                                            std::size_t rd) mutable {
    ...
    void on_read(const boost::system::error_code &ec, response_ptr resp, ssl_socket_ptr ssl_socket, std::size_t rd) {
        ...
        socket_ptr->async_shutdown([this, resp = std::move(resp), ssl_socket = std::move(ssl_socket)](
                                           const boost::system::error_code &ec) mutable {
    ...
    void on_shutdown(const boost::system::error_code &ec, response_ptr resp, ssl_socket_ptr ssl_socket) {
        ...
        if (!m_async_requests.empty()) { async_post(); }
    }
    ...
    void process_reply(const char *fl, int ec, std::string errmsg, std::string body) {
        ...
        __CATCH_BLOCK(std::cout, (std::exception))
    }
...
api::api(boost::asio::io_context &ioctx,
         std::string              host,
         std::string              port,
         std::string              pk,
         std::string              sk,
...
api::result<prices_t::price_t> api::price(const char *symbol, price_cb cb) {
    const impl::init_list_type map = { { "symbol", symbol } };
...
api::result<exchange_info_t> api::exchange_info(const std::vector<std::string> &symbols, exchange_info_cb cb) {
    ...
    for (auto it = symbols.begin(); it != symbols.end(); ++it) {
        ...
        symstr += *it;
        symstr += "\"";
        if (std::next(it) != symbols.end()) { symstr += ","; }
    }
    symstr += "]";

    const impl::init_list_type map = { { "symbols", symstr.c_str() } };
...
api::result<depths_t> api::depths(const char *symbol, std::size_t limit, depths_cb cb) {
    const impl::init_list_type map = { { "symbol", symbol }, { "limit", limit } };
...
api::result<trades_t::trade_t> api::trade(const char *symbol, trade_cb cb) {
    const impl::init_list_type map = { { "symbol", symbol }, { "limit", 1u } };
...
api::result<order_info_t> api::order_info(const char   *symbol,
                                          std::size_t   orderid,
                                          const char   *client_orderid,
                                          order_info_cb cb) {
    const impl::init_list_type map = { { "symbol", symbol },
                                       { "orderId", orderid },
...
api::result<orders_info_t> api::all_orders(const char   *symbol,
                                           std::size_t   orderid,
...
api::result<new_order_resp_type> api::new_order(const char             *symbol,
                                                const e_side            side,
                                                const e_type            type,
                                                const e_time            time,
...
api::result<new_order_resp_type> api::new_test_order(const char             *symbol,
                                                     const e_side            side,
                                                     const e_type            type,
                                                     const e_time            time,
                                                     const e_trade_resp_type resp,
...
api::result<cancel_order_info_t> api::cancel_order(const char     *symbol,
                                                   std::size_t     order_id,
                                                   const char     *client_order_id,
...
api::result<my_trades_info_t> api::my_trades(const char  *symbol,
                                             std::size_t  start_time,
                                             std::size_t  end_time,
                                             std::size_t  from_id,
                                             std::size_t  limit,
...
api::result<ping_user_data_stream_t> api::ping_user_data_stream(const char              *listen_key,
                                                                ping_user_data_stream_cb cb) {
    const impl::init_list_type map = { { "listenKey", listen_key } };
...
api::result<close_user_data_stream_t> api::close_user_data_stream(const char               *listen_key,
                                                                  close_user_data_stream_cb cb) {
    const impl::init_list_type map = { { "listenKey", listen_key } };

    return pimpl->post(false,
                       GET_REST_ROUTE("userDataStream"),
