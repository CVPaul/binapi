...
struct websocket : std::enable_shared_from_this<websocket> {
    ...
    void async_start(const std::string &host,
        ...
        m_resolver.async_resolve(m_host,
                                 ...
                                 [this,
                                  cb     = std::move(cb),
                                  holder = std::move(
                                          holder)](boost::system::error_code                    ec,
  ...
  private:
    ...
    void async_connect(boost::asio::ip::tcp::resolver::results_type res, CB cb, holder_type holder) {
        ...
        boost::asio::async_connect(m_ws.next_layer().next_layer(),
                                   ...
                                   [this,
                                    cb     = std::move(cb),
                                    holder = std::move(holder)](boost::system::error_code ec,
    ...
    void on_connected(CB cb, holder_type holder) {
        m_ws.control_callback(
                [this](boost::beast::websocket::frame_type kind, boost::beast::string_view payload) mutable {
                    (void)kind;
                    (void)payload;
                    // std::cout << "control_callback(" << this << "): kind=" << static_cast<int>(kind) << ",
                    // payload=" << payload.data() << std::endl;
                    m_ws.async_pong(boost::beast::websocket::ping_data{}, [](boost::beast::error_code ec) {
    ...
    void start_read(boost::system::error_code ec, CB cb, holder_type holder) {
        ...
        m_ws.async_read(m_buf,
                        [this, cb = std::move(cb), holder = std::move(holder)](boost::system::error_code ec,
...
struct websockets::impl {
    impl(boost::asio::io_context &ioctx,
        ...
        : m_ioctx{ ioctx }, m_host{ std::move(host) }, m_port{ std::move(port) }, m_on_message{ std::move(msg_cb) },
    ...
    static std::string make_channel_name(const char *pair, const char *channel) {
        ...
        if (pair) {
            res += pair;
            if (*pair != '!') { boost::algorithm::to_lower(res); }
    ...
    websockets::handle start_channel(const char *pair, const char *channel, F cb) {
        ...
        auto wscb = [this, schannel, cb = std::move(cb)](const char *fl,
                                                         int         ec,
                                                         std::string errmsg,
                                                         const char *ptr,
        ...
        auto *ws_ptr = ws.get();
        ws_ptr->async_start(m_host, m_port, schannel, std::move(wscb), std::move(ws));
    ...
    boost::intrusive::set<websocket,
                          boost::intrusive::key_of_value<websocket_id_getter>,
                          boost::intrusive::member_hook<websocket,
...
websockets::websockets(boost::asio::io_context &ioctx,
                       std::string              host,
                       std::string              port,
                       on_message_received_cb   msg_cb,
                       on_network_stat_cb       stat_cb,
...
websockets::handle websockets::part_depth(const char                *pair,
                                          e_levels                   level,
                                          e_freq                     freq,
                                          on_part_depths_received_cb cb) {
    std::string ch = "depth";
...
websockets::handle websockets::userdata(const char          *lkey,
                                        on_account_update_cb account_update,
                                        on_balance_update_cb balance_update,
                                        on_order_update_cb   order_update) {
    auto cb = [acb = std::move(account_update),
               bcb = std::move(balance_update),
               ocb = std::move(
                       order_update)](const char *fl, int ec, std::string errmsg, userdata::userdata_stream_t msg) {
        if (ec) {
